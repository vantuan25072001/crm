...
import BillSelectBoxStep from "@/components/crm/bill/bill_steps/select_box_step";
    ...
    const newElement = (
      <div className={styles.content_obj} key={label}>
        ...
        <div className={styles.choose_obj}>
          ...
          <BillSelectBoxStep value="Tất cả phòng ban" placeholder="Chọn" />
        </div>
      </div>
    );
    if (label === condition) {
  ...
  return (
    <>
      ...
      <ModalCompleteStep
        ...
        title={"Chia sẻ đơn hàng thành công!"}
        link={"#"}
      />
    </>
  );
