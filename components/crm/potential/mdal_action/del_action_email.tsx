...
const DelActionModalEmil: React.FC<MyComponentProps> = ({
  isModalCancel,
  setIsModalCancel,
  record,
}) => {
  ...
  return (
    <>
      ...
      <Modal
        ...
        cancelText="Huỷ"
      >
        <div>Bạn có chắc chắn muốn xóa email {`${record}`}</div>
      </Modal>

      <ModalCompleteStep
        modal1Open={isModalSuccess}
        setModal1Open={setIsMdalSuccess}
        title={`Xóa Email ${record} thành công!`}
        link={"#"}
      />
    </>
  );
...
export default DelActionModalEmil;
